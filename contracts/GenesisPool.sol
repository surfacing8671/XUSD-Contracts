...
contract HamsterGenesisRewardPool {
    ...
    function pendingHAMSTER(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _hamsterReward = _generatedReward * (pool.allocPoint) / (totalAllocPoint);
            accHamsterPerShare = accHamsterPerShare + (_hamsterReward * (1e18) / (tokenSupply));
        }
        return user.amount * (accHamsterPerShare) / (1e18) - (user.rewardDebt);
    }
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return poolEndTime - (poolStartTime) * (hamsterPerSecond);
            return poolEndTime - (_fromTime) * (hamsterPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime) return _toTime - (poolStartTime) * (hamsterPerSecond);
            return _toTime - (_fromTime) * (hamsterPerSecond);
        }
    }
    ...
    constructor(
        ...
        hamsterPerSecond = totalRewards / (runningTime);
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount * (pool.accHamsterPerShare) / (1e18) - (user.rewardDebt);
            if (_pending > 0) {
        ...
        if (amount > 0) {
            ...
            if (_comissionTokens.contains(address(pool.token))) {
                ...
                user.amount = user.amount + (amount * (hundred - (comissionPercent)) / (hundred));
            } else {
                user.amount = user.amount + (amount);
            }
        }
        user.rewardDebt = user.amount * (pool.accHamsterPerShare) / (1e18);
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _hamsterReward = _generatedReward * (pool.allocPoint) / (totalAllocPoint);
            pool.accHamsterPerShare = pool.accHamsterPerShare + (_hamsterReward * (1e18) / (tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function safeHamsterTransfer(address _to, uint256 _amount) internal {
        ...
        if (_hamsterBalance > 0) {
            if (_amount > _hamsterBalance) {
                hamster.transfer(_to, _hamsterBalance);
            } else {
