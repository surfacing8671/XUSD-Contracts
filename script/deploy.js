...
//   const class13 = await ethers.getContractFactory(`LibRegistry2`);
 ...
 // const class3 = await ethers.getContractFactory(`FlatRateTaxCalculator`, {
      ...
      // },
   // });
  //const classReg = await ethers.getContractFactory(`TimeBasedTaxCalculator`, {
  //   libraries: {
  //     AtropaMath: await lib3C.getAddress(),
  //     //VotePass : await lib4C.getAddress(),
  //     LibRegistry: await lib2C.getAddress(),
...
// //   // );
  ...
  const toker = await toke.deploy("d", "d", "18", "1000000000000000000000000000000", await token.getAddress(), {
      })
...
//   // try {
   // let g = await toker.setRegistry(await token.getAddress());
// //     await g.wait();
...
//await classReg1.proposeClass(await userClass2.getAddress(), {     });
  ...
  console.log(
    await t1.balanceOf(signer2.address)
  );
